...
public class LMSValidations {
	...
	public boolean validatedName(String name) throws LMSException {
		String nameRegEx = "^(?=.{4,20}$)(?![.])(?!.*[.]{2})[a-zA-Z0-9.]+(?<![.])";
		...
		if (matcher.matches()) {
			...
			throw new LMSException("Name accepts alphabates and it should have atleast four letters");
		}
		return result;
	...
	public boolean validatedEmail(String email) throws LMSException {
		...
		String emailRegEx = "^[a-z0-9](\\.?[a-z0-9]){4,}@g(oogle)?mail\\.com$";
		boolean result = false;
		...
		if (matcher.matches()) {
			...
			throw new LMSException(
					"Enter proper email,it should contain atleast 5 letters, @, gmail,and end with .com ");
		}
		return result;
	}
